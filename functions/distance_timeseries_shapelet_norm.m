function [D_T_S Xkj_skl]=1-distance_timeseries_shapelet_norm(T,S,alpha)
[mT,nT]=size(T);
DT=T(:,2:end);
[mS,nS]=size(S);
DS=S(:,2:end);
...
for j=1:mT % the j-th time series
    for k=1:mS
        ...
        [X(k,j),Xkj_sk]=distance_longseries_shortseries_norm(time_series,shapelet,alpha); %calculate the distance;
