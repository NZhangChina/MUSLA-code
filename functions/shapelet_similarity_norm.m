function [H XS Hij_sil Parameter]=shapelet_similarity_norm(S,Parameter)
...
for i=1:mS
    ...
    for j=i:mS
        ...
        if length_s <= length_l
            [XS(i,j) XSij_si{i,j}]=distance_longseries_shortseries_norm(sh_l,sh_s,alpha);
        else
            [XS(i,j) XSij_si{i,j}]=distance_longseries_shortseries_norm(sh_s,sh_l,alpha);
        end
